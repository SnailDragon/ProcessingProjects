...
class Maze {
  ...
  public Maze(int rows){
    this.rows = rows;
    this.columns = (int)(rows * width / height);
  ...
  void drawMaze(){
    for(int r = 0; r < getRowCount(); r++){
      for(int c = 0; c < getColumnCount(); c++){
        fill(getCellAt(r,c) * 255);
        rect(c * (float)width / getColumnCount(), r * (float)height / getRowCount(), (float)height / getRowCount(), (float)width / getColumnCount());
      }
    }
  }
}
